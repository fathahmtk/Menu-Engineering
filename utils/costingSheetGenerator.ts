...
export const generateCostingSheetSVG = ({
    ...
    const logoSvg = `<svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="#1d4ed8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`;
    ...
    const htmlContent = `
        <div xmlns="http://www.w3.org/1999/xhtml" style="font-family: 'Poppins', sans-serif; color: #020617; padding: 40px; width: 720px; display: flex; flex-direction: column; height: 100%; background-color: white;">
            
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #1d4ed8; padding-bottom: 20px;">
                <div style="display: flex; align-items: center;">
                    ${logoSvg}
                    <h1 style="font-size: 28px; font-weight: 700; margin: 0 0 0 12px; color: #1d4ed8;">iCAN Costing Sheet</h1>
                </div>
                <div style="text-align: right;">
                    <p style="margin: 0; font-size: 16px; font-weight: 600;">${escapeHTML(business?.name || '')}</p>
                    <p style="margin: 0; font-size: 12px; color: #64748b;">${new Date().toLocaleDateString()}</p>
                </div>
            </div>
            
            <div style="margin-top: 30px; text-align: center;">
                <h2 style="font-size: 32px; font-weight: 700; margin: 0; color: #020617;">${escapeHTML(recipe.name)}</h2>
                <p style="font-size: 16px; color: #475569; margin: 4px 0 0 0;">Category: ${escapeHTML(recipe.category)}</p>
            </div>
            ...
            <div style="margin-top: 30px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
                <div style="background-color: #f8fafc; padding: 16px; border-radius: 8px; border: 1px solid #e2e8f0; text-align: center;">
                    <p style="font-size: 14px; color: #64748b; margin: 0 0 4px 0;">Total Recipe Cost</p>
                    <p style="font-size: 24px; font-weight: 700; color: #1d4ed8; margin: 0;">${formatCurrency(totalCost)}</p>
                </div>
                <div style="background-color: #f8fafc; padding: 16px; border-radius: 8px; border: 1px solid #e2e8f0; text-align: center;">
                    <p style="font-size: 14px; color: #64748b; margin: 0 0 4px 0;">Servings</p>
                    <p style="font-size: 24px; font-weight: 700; margin: 0;">${recipe.servings}</p>
                </div>
                 <div style="background-color: #f8fafc; padding: 16px; border-radius: 8px; border: 1px solid #e2e8f0; text-align: center;">
                    <p style="font-size: 14px; color: #64748b; margin: 0 0 4px 0;">Cost per Serving</p>
             ...
             <div style="margin-top: 20px; display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
                 <div style="background-color: #eff6ff; padding: 16px; border-radius: 8px; border: 1px solid #dbeafe; text-align: center;">
                    <p style="font-size: 14px; color: #1e40af; margin: 0 0 4px 0;">Suggested Sale Price (30% Cost)</p>
                    <p style="font-size: 24px; font-weight: 700; color: #1d4ed8; margin: 0;">${formatCurrency(suggestedPrice)}</p>
                </div>
                 <div style="background-color: #f8fafc; padding: 16px; border-radius: 8px; border: 1px solid #e2e8f0; text-align: center;">
                    <p style="font-size: 14px; color: #64748b; margin: 0 0 4px 0;">Target Sale Price</p>
                    <p style="font-size: 24px; font-weight: 700; margin: 0;">${formatCurrency(recipe.targetSalePricePerServing || 0)}</p>
                </div>
            </div>

            <div style="margin-top: 30px;">
                <h3 style="font-size: 20px; font-weight: 600; margin-bottom: 12px; color: #020617;">Ingredient Breakdown</h3>
            ...
            <div style="margin-top: auto; padding-top: 20px; border-top: 1px solid #e2e8f0; text-align: center; font-size: 12px; color: #64748b;">
